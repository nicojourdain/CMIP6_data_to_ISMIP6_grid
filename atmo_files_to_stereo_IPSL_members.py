...
#scenar_list = ['historical','ssp126','ssp245','ssp585','piControl','hist-aer', 'hist-nat','hist-GHG','hist-stratO3','ssp245-nat','ssp245-GHG']
scenar_list = ['hist-aer']
#var_list=['tas','pr','evspsbl','mrro','mrros']
var_list=['tas']

for model in model_list:

  #for NN in np.arange(1,34,1):
  for NN in np.arange(1,11,1):
    ...
    for scenar in scenar_list:
        ...
        for var in var_list:
            ...
            if ( ( scenar[0:5] == 'hist-' ) | ( scenar[0:7] == 'ssp245-' ) ):
                var_dir=cmip_dir+'/DAMIP/'+institute+'/'+model+'/'+scenar+'/'+member+'/Amon/'+var+'/'+grd+'/latest'
            elif ( scenar[0:3] == 'ssp' ):
