...
&          molevel, mnvertex, my, mx, maxis_nbounds, time_bounds_ID, time_ID, so_ID, olevel_bounds_ID,     &
&          olevel_ID, area_ID, bounds_nav_lat_ID, bounds_nav_lon_ID, nav_lon_ID, nav_lat_ID, fidM, kmem, Nmemb,&
&          Nmembers, so_mean_ID, kt
...
REAL*4,ALLOCATABLE,DIMENSION(:,:,:) :: bounds_nav_lat, bounds_nav_lon, so, so_mean
...
222 FORMAT('/scratchu/njourdain/MULTI_MEMBER_MEAN/so_Omon_IPSL-CM6A-LR_historical_r0i1p1f1_gn_',a,'.nc')
...
101 FORMAT('/bdd/CMIP6/CMIP/IPSL/IPSL-CM6A-LR/historical-EXT/r',i1,'i1p1f1/Omon/so/gn/latest/so_Omon_IPSL-CM6A-LR_historical_r',i1,'i1p1f1_gn_',a,'.nc')
102 FORMAT('/bdd/CMIP6/CMIP/IPSL/IPSL-CM6A-LR/historical-EXT/r',i2,'i1p1f1/Omon/so/gn/latest/so_Omon_IPSL-CM6A-LR_historical_r',i2,'i1p1f1_gn_',a,'.nc')
...
status = NF90_INQ_DIMID(fidA,"olevel",dimID_olevel)
if ( status .ne. 0 ) status = NF90_INQ_DIMID(fidA,"lev",dimID_olevel)
call erreur(status,.TRUE.,"inq_dimID_olevel")
...
ALLOCATE(  time(mtime)  )
ALLOCATE(  so(mx,my,molevel)  )
ALLOCATE(  so_mean(mx,my,molevel)  )
...
status = NF90_DEF_VAR(fidM,"so",NF90_FLOAT,(/dimID_x,dimID_y,dimID_olevel,dimID_time/),so_mean_ID,deflate_level=1); call erreur(status,.TRUE.,"def_var_so_mean_ID")
...
status = NF90_PUT_ATT(fidM,so_mean_ID,"history","none"); call erreur(status,.TRUE.,"put_att_so_mean_ID")
status = NF90_PUT_ATT(fidM,so_mean_ID,"description","Diagnostic should be contributed even for models using conservative temperature as prognostic field."); call erreur(status,.TRUE.,"put_att_so_mean_ID")
status = NF90_PUT_ATT(fidM,so_mean_ID,"coordinates","nav_lat nav_lon"); call erreur(status,.TRUE.,"put_att_so_mean_ID")
status = NF90_PUT_ATT(fidM,so_mean_ID,"missing_value",1.e+20); call erreur(status,.TRUE.,"put_att_so_mean_ID")
status = NF90_PUT_ATT(fidM,so_mean_ID,"_FillValue",1.e+20); call erreur(status,.TRUE.,"put_att_so_mean_ID")
status = NF90_PUT_ATT(fidM,so_mean_ID,"cell_measures","area: areacello volume: volcello"); call erreur(status,.TRUE.,"put_att_so_mean_ID")
status = NF90_PUT_ATT(fidM,so_mean_ID,"interval_write","1 month"); call erreur(status,.TRUE.,"put_att_so_mean_ID")
status = NF90_PUT_ATT(fidM,so_mean_ID,"interval_operation","2700 s"); call erreur(status,.TRUE.,"put_att_so_mean_ID")
status = NF90_PUT_ATT(fidM,so_mean_ID,"cell_methods","area: mean where sea time: mean"); call erreur(status,.TRUE.,"put_att_so_mean_ID")
status = NF90_PUT_ATT(fidM,so_mean_ID,"online_operation","average"); call erreur(status,.TRUE.,"put_att_so_mean_ID")
status = NF90_PUT_ATT(fidM,so_mean_ID,"units","0.001"); call erreur(status,.TRUE.,"put_att_so_mean_ID")
status = NF90_PUT_ATT(fidM,so_mean_ID,"long_name","Sea Water Salinity"); call erreur(status,.TRUE.,"put_att_so_mean_ID")
status = NF90_PUT_ATT(fidM,so_mean_ID,"standard_name","sea_water_salinity"); call erreur(status,.TRUE.,"put_att_so_mean_ID")
...
status = NF90_PUT_ATT(fidM,NF90_GLOBAL,"history","Created using build_multimember_mean_so_IPSL.f90")
...
DO kt=1,mtime
  ...
  do kmem=1,Nmembers
   if ( kmem .ne. 2 .and. kmem .ne. 16 ) then  ! IPSL
    ...
    status = NF90_INQ_VARID(fidA,"so",so_ID); call erreur(status,.TRUE.,"inq_so_ID")
    status = NF90_GET_VAR(fidA,so_ID,so,start=(/1,1,1,kt/),count=(/mx,my,molevel,1/)); call erreur(status,.TRUE.,"getvar_so")
  ...
  so_mean(:,:,:) = so_mean(:,:,:) / Nmemb
  
  status = NF90_PUT_VAR(fidM,so_mean_ID,so_mean,start=(/1,1,1,kt/),count=(/mx,my,molevel,1/))
  call erreur(status,.TRUE.,"var_so_mean_ID")
